...
export default function BrowseMode({ activeProjectId, setMode }: BrowseModeProps) {
  ...
  // ── TEMPORARY CLEANUP ── remove after user runs once ──────────────────────
  const CLEANUP_NAMES = new Set(["MAGIC LAS VEGAS","ASD Market Week","Cosmoprof North America-Las Vegas","Las Vegas Market Show","AAPEX","Apparel Sourcing New York City","NY NOW","The Energy Expo","COTERIE New York","Greater New York Dental Meeting","SOURCING by Informa","GTM North America","Florida' s Largest Home Show","SuperZoo","The Water Expo","IMEX America","International Conference on Agricultural and Biological Systems Engineering","World Dairy Expo","International Fashion Jewellery & Accessory","IAAPA Expo","Virtuoso Travel Week","Americas Food and Beverage Show","The Travel Healthcare Conference","Home Textiles Sourcing New York City","Home Design and Remodeling Show","Houston Build Expo","Washington Journalism and Media Conference","Los Angeles Build Expo","RISE Women's Leadership Conference","MJBizCon","The International Congress of Esthetics & Spa Long Beach","SEMA Show","DEFCON","Printsource New York","EnerCom Denver","PROJECT Las Vegas","Southwest Believers Convention","TechCrunch Disrupt","Fall Home Expo","American Geophysical Union Fall Meeting","Womenswear in Nevada","Paper & Plastics Recycling Conference","Coffee Fest New York","AWS Re:Invent","Safety & Health Conference","International Esthetics, Cosmetics & SPA Conference","American Sociological Association Meeting","Los Angeles Womens Expo","High Point Market","International Gem & Jewelry Show","Dance Teacher Web Conference & Expo","Insurance Conference","Outdoor Retailer","Florida Realtors Convention & Trade Expo","Make Up in New York","International Woodworking Fair","ISC East","Minnesota Farmfest","MAGIC Men's","Antibody Engineering & Therapeutics Conference","SEMICON West","VMworld","Dallas Total Home & Gift Market","New York City Jewelry & Watch Show","Global Gaming Expo","Surf Expo","Fort Lauderdale International Boat Show","Trendz Show","Massachusetts Conference for Women","Boutique Design New York","Tri-State Seminar","Solar Power International","Great Food Expo","AUSA Annual Meeting and Exposition","Annual MAS Convention","PACK EXPO International","NAB Show New York","South Florida Build Expo","Ai4","Charlotte Build Expo","The Global Leadership Summit","Franchise Expo West","EDspaces","Art Miami","Impressions Expo","IGES in Sevierville","Pizza Expo Columbus","WR Expo - The Waste & Recycling Tradeshow for The Americas","Digital Dealer Conference & Expo","CEDIA Expo & Commercial Integrator Expo","A4M/MMI Annual World Congress","Turbomachinery & Pump Symposia","World Travel Expo","ISSA Show North America","Atlanta Jewelry Show","Premiere Columbus","Functional Fabric Fair","Groom Expo","Great Food Expo","AI Summit New York","Gem & Lapidary Wholesalers Show-Tucson","International Conference on Environment and Natural Science","Fall Agribusiness Conference","MAGIC New York","Design Chicago","Affordable Art Fair New York","The Atlanta Shoe Market","SupplySide Global","EAA AirVenture Oshkosh","New York City Technology Forum","World Business Forum","HR Florida Conference & Expo","VEITH Symposium","The Lodging Conference","NYC Real Estate Week","Southeast Building Conference","Chicago Build","PLMA's Annual Private Label Trade Show","Health & Fitness Expo","Infectious Disease in Children Symposium","Construction Financial Management Conference","For The Church National Conference","NPMA Pestworld","The Battery Show","Human Resource Technology Conference and Exposition","Pacific Marine Expo","American Towman Exposition","GlassBuild America","Silicon Valley International Invention Festival","NW Materials Show","Atlanta International Auto Show","International Quilt Market Houston","LOUPE Americas","Greensboros Holiday Market","Team Church Conference","International Citrus & Beverage Conference","Clinical Lab Expo","Mental Health America Conference","Wedding MBA","Black Hat USA","PRINTING United Expo","LRA Showcase","Central Florida Home Expo","Man/Woman New York","International Conference on Wireless Networks","Interservice/Industry Training, Simulation and Education Conference","Groundwater Week","Electric & Hybrid Vehicle Technology Expo","NJEA Convention","International LGBTQ Leaders Conference"].map(n => n.toLowerCase()));

  const [cleanupStatus, setCleanupStatus] = useState<'idle' | 'running' | 'done'>('idle');
  const [cleanupResult, setCleanupResult] = useState('');
  const [orphanStatus, setOrphanStatus] = useState<'idle' | 'running' | 'done'>('idle');
  const [orphanResult, setOrphanResult] = useState('');

  const handleCleanupBadImport = async () => {
    if (!user) return;
    setCleanupStatus('running');
    let deleted = 0;

    try {
      // Delete matching claimed_leads
      const claimSnap = await getDocs(fsQuery(collection(db, 'claimed_leads'), where('claimedBy', '==', user.uid)));
      for (const d of claimSnap.docs) {
        if (CLEANUP_NAMES.has((d.data().eventName || '').toLowerCase())) {
          await deleteDoc(d.ref);
          deleted++;
        }
      }

      // Delete matching research_cue and scans across all projects
      const projSnap = await getDocs(collection(db, 'users', user.uid, 'projects'));
      for (const projDoc of projSnap.docs) {
        const pid = projDoc.id;
        const [cueSnap, scanSnap] = await Promise.all([
          getDocs(collection(db, 'users', user.uid, 'projects', pid, 'research_cue')),
          getDocs(collection(db, 'users', user.uid, 'projects', pid, 'scans')),
        ]);
        for (const d of [...cueSnap.docs, ...scanSnap.docs]) {
          if (CLEANUP_NAMES.has((d.data().eventName || '').toLowerCase())) {
            await deleteDoc(d.ref);
            deleted++;
          }
      ...
      setCleanupResult(`Done — deleted ${deleted} documents. You can now re-import.`);
    } catch (err) {
      console.error(err);
      setCleanupResult('Error during cleanup — check the console.');
    }
    setCleanupStatus('done');
  };
  const handleRebuildClaims = async () => {
    if (!user) return;
    setOrphanStatus('running');
    try {
      // Get existing claimed_leads so we don't duplicate
      const existingSnap = await getDocs(fsQuery(collection(db, 'claimed_leads'), where('claimedBy', '==', user.uid)));
      const existingNames = new Set(existingSnap.docs.map(d => (d.data().normalizedName || '')));
      ...
      for (const projDoc of projSnap.docs) {
        ...
        // Rebuild from pipeline events
        const eventsSnap = await getDocs(collection(db, 'users', user.uid, 'projects', pid, 'events'));
        for (const d of eventsSnap.docs) {
          const ev = d.data();
          const nn = normalizeName(ev.eventName || '');
          if (nn && !existingNames.has(nn)) {
            await addDoc(collection(db, 'claimed_leads'), {
              eventName: ev.eventName || '',
              website: ev.website || '',
              normalizedDomain: normalizeDomain(ev.website || ''),
              normalizedName: nn,
              searchType: ev.searchType || 'event',
              claimedBy: user.uid,
              claimedByName: user.displayName || user.email || '',
              claimedAt: serverTimestamp(),
              status: ev.status || 'Initial',
              responseOutcome: ev.responseOutcome || null,
            });
        ...
        // Rebuild from research cue
        const cueSnap = await getDocs(collection(db, 'users', user.uid, 'projects', pid, 'research_cue'));
        for (const d of cueSnap.docs) {
          const ev = d.data();
          const nn = normalizeName(ev.eventName || '');
          if (nn && !existingNames.has(nn)) {
            await addDoc(collection(db, 'claimed_leads'), {
      ...
      setOrphanResult(`Rebuilt ${rebuilt} missing claimed lead${rebuilt !== 1 ? 's' : ''} from your pipeline and cue.`);
    } catch (err) {
      console.error(err);
      setOrphanResult('Error — check the console.');
    }
      ...
      <main className="flex-1 overflow-hidden flex flex-col glass rounded-2xl h-full border border-white/10 select-text p-6">
        ...
        {browseSubTab === 'import' ? (
          <div className="flex-1 flex flex-col overflow-hidden gap-4">
            ...
            {/* ── TEMPORARY CLEANUP BANNER ── remove after use ── */}
            <div className="shrink-0 bg-amber-500/10 border border-amber-500/25 rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-xs font-bold text-amber-300">Clean Up Bad Import (150 leads)</p>
                  <p className="text-[10px] text-slate-400 mt-0.5">Delete the 150 leads from the bad import so you can re-import with full data.</p>
                  {cleanupResult && <p className="text-[10px] text-green-400 mt-1 font-semibold">{cleanupResult}</p>}
                </div>
                <button
                  type="button"
                  onClick={handleCleanupBadImport}
                  disabled={cleanupStatus !== 'idle'}
                  className="shrink-0 px-3 py-1.5 bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/30 text-amber-300 text-xs font-bold rounded-lg cursor-pointer transition-all disabled:opacity-50 disabled:pointer-events-none flex items-center gap-1.5"
                >
                  {cleanupStatus === 'running' ? <><RefreshCw className="h-3.5 w-3.5 animate-spin" /> Cleaning...</> : cleanupStatus === 'done' ? <><Check className="h-3.5 w-3.5" /> Done</> : 'Run Cleanup'}
                </button>
              </div>
              <div className="flex items-center justify-between gap-4 border-t border-white/5 pt-3">
                <div>
                  <p className="text-xs font-bold text-amber-300">Rebuild Missing Claims</p>
                  <p className="text-[10px] text-slate-400 mt-0.5">Re-creates claimed_lead entries for anything in your pipeline or cue that lost its claim record.</p>
                  {orphanResult && <p className="text-[10px] text-green-400 mt-1 font-semibold">{orphanResult}</p>}
                </div>
                <button
                  type="button"
                  onClick={handleRebuildClaims}
                  disabled={orphanStatus !== 'idle'}
                  className="shrink-0 px-3 py-1.5 bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/30 text-amber-300 text-xs font-bold rounded-lg cursor-pointer transition-all disabled:opacity-50 disabled:pointer-events-none flex items-center gap-1.5"
                >
                  {orphanStatus === 'running' ? <><RefreshCw className="h-3.5 w-3.5 animate-spin" /> Rebuilding...</> : orphanStatus === 'done' ? <><Check className="h-3.5 w-3.5" /> Done</> : 'Rebuild Claims'}
                </button>
              </div>
            </div>
            {/* ── END TEMPORARY CLEANUP BANNER ── */}
